...
if (isset($_POST['add_product'])) {
    ...
    // Multiple photos: name="product_images[]" — the first valid one becomes the cover photo
    // used everywhere else in the site (grids, cart, orders); all of them go into the gallery.
    if (isset($_FILES['product_images']) && is_array($_FILES['product_images']['name'])) {
        $allowed = ['jpg','jpeg','png','gif','webp'];
        foreach ($_FILES['product_images']['name'] as $i => $filename) {
            if ($_FILES['product_images']['error'][$i] !== 0 || $filename === '') continue;
            $filetype = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
            if (!in_array($filetype, $allowed)) { $error = "Invalid file type: $filename"; continue; }
            $saved_name = 'prod_' . uniqid() . '_' . basename($filename);
            if (move_uploaded_file($_FILES['product_images']['tmp_name'][$i], $upload_dir . $saved_name)) {
                $uploaded_images[] = $saved_name;
            } else {
                $error = "Failed to upload image: $filename";
            }
        }
        if (!empty($uploaded_images)) $image_name = $uploaded_images[0];
    }
    ...
    if (empty($error)) {
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            foreach ($uploaded_images as $order => $img) {
                $img_safe = mysqli_real_escape_string($conn, $img);
                $is_primary = $order === 0 ? 1 : 0;
                mysqli_query($conn, "INSERT INTO product_images (product_id, image_url, display_order, is_primary) VALUES ($new_product_id, '$img_safe', $order, $is_primary)");
            }
            $success = "Product submitted for review! It will be live once approved.";
        }
...
if (isset($_POST['add_images']) && isset($_POST['product_id'])) {
    $pid = intval($_POST['product_id']);
    $owns = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id, image_url FROM products WHERE id = $pid AND manufacturer_id = $manufacturer_id"));
    if ($owns && isset($_FILES['extra_images']) && is_array($_FILES['extra_images']['name'])) {
        $allowed = ['jpg','jpeg','png','gif','webp'];
        $existing_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM product_images WHERE product_id = $pid"))['c'];
        $next_order = (int)$existing_count;
        $added = 0;
        foreach ($_FILES['extra_images']['name'] as $i => $filename) {
            if ($_FILES['extra_images']['error'][$i] !== 0 || $filename === '') continue;
            $filetype = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
            if (!in_array($filetype, $allowed)) continue;
            $saved_name = 'prod_' . uniqid() . '_' . basename($filename);
            if (move_uploaded_file($_FILES['extra_images']['tmp_name'][$i], $upload_dir . $saved_name)) {
                $img_safe = mysqli_real_escape_string($conn, $saved_name);
                $is_primary = (empty($owns['image_url']) && $added === 0) ? 1 : 0;
                mysqli_query($conn, "INSERT INTO product_images (product_id, image_url, display_order, is_primary) VALUES ($pid, '$img_safe', $next_order, $is_primary)");
                if (empty($owns['image_url']) && $added === 0) {
                    mysqli_query($conn, "UPDATE products SET image_url = '$img_safe' WHERE id = $pid");
                }
                $next_order++; $added++;
            }
        }
        $success = $added > 0 ? "$added photo(s) added." : "No photos were added.";
    }
...
if (isset($_GET['delete'])) {
    ...
    $imgs = mysqli_query($conn, "SELECT image_url FROM product_images WHERE product_id = $id");
    while ($imgs && $img = mysqli_fetch_assoc($imgs)) {
...
$categories = mysqli_query($conn, "SELECT * FROM categories WHERE is_active = 1 ORDER BY pillar ASC, name ASC");
$pillar_labels = ['baby_gear' => '🧸 Baby Gear', 'nutrition' => '🍎 Nutrition', 'parenting' => '👶 Parenting'];
          ...
          <form method="POST" action="manufacturer_dashboard.php?tab=products" enctype="multipart/form-data">
            <div class="form-grid">
                ...
                <select name="category_id" class="form-control" required>
                  ...
                  <?php
                    ...
                    while ($cat = mysqli_fetch_assoc($categories)):
                      if ($cat['pillar'] !== $current_pillar):
                        if ($current_pillar !== null) echo '</optgroup>';
                        $current_pillar = $cat['pillar'];
                        echo '<optgroup label="' . htmlspecialchars($pillar_labels[$current_pillar] ?? $current_pillar) . '">';
                      endif;
                  ?>
                  <option value="<?= $cat['id'] ?>"><?= htmlspecialchars($cat['name']) ?></option>
                  <?php endwhile; if ($current_pillar !== null) echo '</optgroup>'; ?>
                </select>
              </div>
              ...
              <div class="form-group full">
                <label>Product Photos <span style="font-weight:400;text-transform:none;letter-spacing:0">(you can select several at once)</span></label>
                <div class="file-zone" onclick="document.getElementById('productImg').click()">
                  <input type="file" name="product_images[]" id="productImg" accept="image/*" multiple onchange="previewImgs(this)"/>
                  <div style="font-size:32px;margin-bottom:8px;">📸</div>
                  <div style="font-size:14px;font-weight:600;color:var(--text-mid);">Click to upload photos</div>
                  <div style="font-size:12px;color:var(--text-soft);margin-top:4px;">JPG, PNG, WEBP — max 5 MB each. First photo becomes the cover photo.</div>
                </div>
                <div class="preview-box" id="previewBox" style="margin-top:14px;display:none;gap:10px;flex-wrap:wrap;"></div>
              </div>
            </div>
          ...
          <table>
            ...
            <tbody>
              <?php while ($p = mysqli_fetch_assoc($products)):
                $img_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM product_images WHERE product_id = " . (int)$p['id']))['c'];
              ?>
              <tr>
                <td>
                  <div style="display:flex;align-items:center;gap:14px;">
                    ...
                    <div>
                      ...
                      <div style="font-size:12px;color:var(--text-soft);">#<?= $p['id'] ?> · <?= (int)$img_count ?> photo<?= $img_count == 1 ? '' : 's' ?></div>
                      <label style="font-size:11px;color:var(--rose-dark);cursor:pointer;text-decoration:underline;" onclick="document.getElementById('addImgInput<?= $p['id'] ?>').click()">+ Add photos</label>
                      <form method="POST" action="manufacturer_dashboard.php?tab=products" enctype="multipart/form-data" id="addImgForm<?= $p['id'] ?>" style="display:none;">
                        <input type="hidden" name="product_id" value="<?= $p['id'] ?>"/>
                        <input type="file" name="extra_images[]" id="addImgInput<?= $p['id'] ?>" accept="image/*" multiple onchange="document.getElementById('addImgForm<?= $p['id'] ?>').submit()"/>
    ...
    if (input.files && input.files.length) {
      ...
      Array.from(input.files).forEach((file, i) => {
        ...
        reader.onload = e => {
          ...
          img.style.cssText = 'width:100px;height:100px;object-fit:cover;border-radius:12px;border:2px solid var(--border-light);';
          wrap.appendChild(img);
          if (i === 0) {
            const tag = document.createElement('div');
            tag.textContent = 'Cover';
            tag.style.cssText = 'position:absolute;top:4px;left:4px;background:var(--rose-dark);color:#fff;font-size:10px;font-weight:700;padding:2px 6px;border-radius:6px;';

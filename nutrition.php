...
// ── Products (Nutrition pillar only) ─────────────────────────
$where = "p.status='active' AND p.is_active=1 AND c.pillar='nutrition'";
...
$categories = mysqli_query($conn, "SELECT * FROM categories WHERE is_active=1 AND pillar='nutrition' ORDER BY name ASC");
 
$manufacturers = mysqli_query($conn, "
  SELECT DISTINCT m.id, m.company_name
  FROM manufacturers m
  JOIN products p ON p.manufacturer_id = m.id
  JOIN categories c ON p.category_id = c.id AND c.pillar='nutrition'
  WHERE m.verification_status='verified' AND m.is_active=1
  ORDER BY m.company_name ASC
");
 
$cat_counts = [];
$cc = mysqli_query($conn, "SELECT p.category_id, COUNT(*) cnt FROM products p JOIN categories c ON p.category_id=c.id WHERE c.pillar='nutrition' AND p.status='active' AND p.is_active=1 GROUP BY p.category_id");
while ($cc && $r = mysqli_fetch_assoc($cc)) $cat_counts[$r['category_id']] = (int)$r['cnt'];
 
$brand_counts = [];
$bc = mysqli_query($conn, "SELECT p.manufacturer_id, COUNT(*) cnt FROM products p JOIN categories c ON p.category_id=c.id WHERE c.pillar='nutrition' AND p.status='active' AND p.is_active=1 GROUP BY p.manufacturer_id");
while ($bc && $r = mysqli_fetch_assoc($bc)) $brand_counts[$r['manufacturer_id']] = (int)$r['cnt'];
 
$rating_counts = [];
foreach ([5,4,3,2,1] as $star) {
    $rc = mysqli_query($conn, "SELECT COUNT(*) cnt FROM products p JOIN categories c ON p.category_id=c.id WHERE c.pillar='nutrition' AND p.status='active' AND p.is_active=1 AND p.rating >= $star");
...
function placeholderPhoto($product_id, $keyword = 'babyfood') {
    return "https://loremflickr.com/500/500/" . $keyword . "?lock=" . intval($product_id);
}
 
// ── Articles (Nutrition guidance content) ─────────────────────
$articles = mysqli_query($conn, "SELECT * FROM articles WHERE pillar='nutrition' AND is_published=1 ORDER BY created_at DESC");
...
<head>
  ...
  <title>Nutrition – BabyBliss</title>
  <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700&family=DM+Sans:wght@300;400;500;600&display=swap" rel="stylesheet"/>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css"/>
  <style>
    :root { --cream:#FFF8F0; --rose:#E8738A; --deep-rose:#C44D65; --mint:#A8D8C8; --mint-dark:#5FB8A0; --mint-deep:#3A9E88; --blue:#6678CC; --gold:#F5C842; --white:#FFFFFF; --text-dark:#2D1B14; --text-mid:#6B4C3B; --text-light:#A07D6A; --shadow:rgba(95,184,160,0.15); --radius:20px; }
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .header-top { background:var(--mint-deep); text-align:center; padding:6px; font-size:13px; color:var(--white); }
    .header-main { display:flex; align-items:center; justify-content:space-between; padding:0 48px; height:72px; }
    .logo { display:flex; align-items:center; gap:10px; text-decoration:none; }
    .logo-icon { width:44px; height:44px; background:linear-gradient(135deg,var(--mint),var(--mint-deep)); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:22px; }
    .logo-text { font-family:'Playfair Display', serif; font-size:26px; font-weight:700; color:var(--mint-deep); line-height:1; }
    .search-bar { flex:1; max-width:420px; margin:0 24px; position:relative; }
    .search-bar input { width:100%; padding:12px 20px 12px 44px; border:2px solid #E0F0EA; border-radius:12px; font-size:14px; outline:none; }
    .search-bar input:focus { border-color:var(--mint-dark); box-shadow:0 0 0 4px rgba(95,184,160,.12); }
    ...
    .pillar-link.nutrition { background:var(--mint-deep); color:#fff; }
    ...
    .icon-btn:hover { background:var(--cream); color:var(--mint-deep); }
    .badge { position:absolute; top:2px; right:2px; background:var(--mint-deep); color:var(--white); font-size:10px; font-weight:600; width:18px; height:18px; border-radius:50%; display:flex; align-items:center; justify-content:center; }
    .btn-login { padding:9px 20px; border-radius:10px; font-size:14px; font-weight:600; border:2px solid var(--mint-dark); color:var(--mint-deep); background:transparent; text-decoration:none; }
    .btn-login:hover { background:var(--mint-dark); color:var(--white); }
    .btn-sell { padding:9px 20px; border-radius:10px; font-size:14px; font-weight:600; border:none; background:linear-gradient(135deg,var(--mint-dark),var(--mint-deep)); color:var(--white); text-decoration:none; }
 
    .pillar-hero { background:linear-gradient(135deg,#0F2A22 0%,#1A4A3A 50%,#2A6A52 100%); padding:50px 48px; display:flex; align-items:center; justify-content:space-between; position:relative; overflow:hidden; }
    .hero-content { position:relative; z-index:1; max-width:600px; }
    .hero-tag { display:inline-block; background:var(--mint-dark); color:white; padding:4px 14px; border-radius:20px; font-size:11px; font-weight:600; letter-spacing:2px; text-transform:uppercase; margin-bottom:16px; }
    ...
    /* TABS */
    .tabs-wrap { background:var(--white); border-bottom:1px solid #E0F0EA; padding:0 48px; }
    ...
    .tab:hover { color:var(--mint-deep); }
    .tab.active { color:var(--mint-deep); border-bottom-color:var(--mint-deep); }
    ...
    .switch-item.nutrition { background:linear-gradient(135deg,#EAF8F4,#D4F0E6); color:var(--mint-deep); }
    .switch-item.parenting { color:var(--text-mid); } .switch-item.parenting:hover { background:#EEF0FB; color:var(--blue); }
    ...
    .filter-panel-header a { font-size:12px; font-weight:600; color:var(--mint-deep); text-decoration:none; }
    ...
    .filter-group-header span.label .tick { color:var(--mint-deep); }
    .filter-group-header a.clear-link { font-size:11px; color:var(--text-light); text-decoration:underline; }
    .filter-group-header a.clear-link:hover { color:var(--mint-deep); }
    ...
    .filter-checkbox-item:hover { color:var(--mint-deep); }
    .filter-checkbox-item input[type=checkbox], .filter-checkbox-item input[type=radio] { width:16px; height:16px; accent-color:var(--mint-dark); cursor:pointer; flex-shrink:0; }
    ...
    .price-range input { width:100%; padding:8px 12px; border:2px solid #E0F0EA; border-radius:8px; font-size:13px; }
    .price-range button { padding:8px 16px; background:var(--mint-dark); color:white; border:none; border-radius:8px; font-size:13px; font-weight:600; cursor:pointer; }
    ...
    .filter-item:hover { background:var(--cream); color:var(--mint-deep); }
    .filter-item.active { background:linear-gradient(135deg,#EAF8F4,#D4F0E6); color:var(--mint-deep); font-weight:600; }
    ...
    .sort-bar select { padding:8px 16px; border:2px solid #E0F0EA; border-radius:10px; font-size:14px; background:var(--white); cursor:pointer; outline:none; }
    ...
    .product-card:hover { transform:translateY(-4px); box-shadow:0 16px 40px rgba(95,184,160,.18); border-color:var(--mint); }
    .product-badge { position:absolute; top:12px; left:12px; background:var(--mint-deep); color:var(--white); font-size:10px; font-weight:700; padding:4px 10px; border-radius:6px; z-index:2; text-transform:uppercase; }
    .product-img { width:100%; height:200px; background:linear-gradient(135deg,#EAF8F4,#D4F0E6); display:flex; align-items:center; justify-content:center; font-size:80px; overflow:hidden; }
    .product-img img { width:100%; height:100%; object-fit:cover; }
    .product-body { padding:16px; }
    .product-cat { font-size:11px; font-weight:700; color:var(--mint-dark); text-transform:uppercase; letter-spacing:1px; }
    ...
    .price-current { font-size:20px; font-weight:700; color:var(--mint-deep); }
    .price-old { font-size:13px; color:var(--text-light); text-decoration:line-through; }
    .btn-cart { width:100%; padding:10px; background:linear-gradient(135deg,var(--mint-dark),var(--mint-deep)); color:var(--white); border:none; border-radius:10px; font-size:13px; font-weight:600; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:6px; text-decoration:none; margin:0 16px 16px; }
    ...
    .article-card:hover { transform:translateY(-4px); box-shadow:0 14px 32px rgba(95,184,160,.2); }
    .article-icon { width:60px; height:60px; border-radius:14px; background:linear-gradient(135deg,#EAF8F4,#D4F0E6); display:flex; align-items:center; justify-content:center; font-size:30px; flex-shrink:0; }
    .article-topic { font-size:11px; font-weight:700; color:var(--mint-deep); text-transform:uppercase; letter-spacing:1px; margin-bottom:6px; }
    ...
    footer { background:#0F2A22; color:rgba(255,255,255,.8); margin-top:40px; }
...
<header>
  <div class="header-top">🍎 Nutrition for Growing Bodies · Expert-Reviewed Guidance · Trusted Brands</div>
  <div class="header-main">
    <a href="index.php" class="logo">
      <div class="logo-icon">🍎</div>
      <div class="logo-text">BabyBliss</div>
    </a>
    <form class="search-bar" action="nutrition.php" method="GET">
      <i class="fas fa-search"></i>
      <input type="text" name="search" placeholder="Search nutrition products..." value="<?= htmlspecialchars($search) ?>"/>
    </form>
...
<div class="pillar-hero">
  <div class="hero-content">
    <div class="hero-tag">🍎 Nutrition Pillar</div>
    <h1>Feed Your Baby with Confidence</h1>
    <p>Formula, vitamins, and feeding essentials from trusted sellers — paired with expert-reviewed articles on weaning, hydration, and healthy growth.</p>
  </div>
  ...
  <aside>
    ...
    <div class="sidebar-filters">
      <div class="filter-panel-header">
        <span>FILTER BY</span>
        <?php if ($has_active_filters): ?><a href="nutrition.php">Clear All</a><?php endif; ?>
      </div>
 
      <form method="GET" action="nutrition.php" id="filterForm">
        ...
        <div class="filter-group">
          <div class="filter-group-header">
            ...
            <?php if ($category_filter): ?><a class="clear-link" href="<?= buildFilterUrl('nutrition.php', ['category' => null]) ?>">Clear</a><?php endif; ?>
          </div>
        ...
        <div class="filter-group">
          <div class="filter-group-header">
            ...
            <?php if ($manufacturer_filter): ?><a class="clear-link" href="<?= buildFilterUrl('nutrition.php', ['manufacturer' => null]) ?>">Clear</a><?php endif; ?>
          </div>
          <div class="filter-checkbox-list">
        ...
        <div class="filter-group">
          <div class="filter-group-header">
            ...
            <?php if ($rating_filter): ?><a class="clear-link" href="<?= buildFilterUrl('nutrition.php', ['rating' => null]) ?>">Clear</a><?php endif; ?>
          </div>
    ...
    <div class="sidebar-filters">
      ...
      <div class="filter-group">
        <div class="filter-checkbox-list short" style="gap:8px;">
          <span class="filter-item active"><i class="fas fa-utensils"></i> Feeding</span>
          <span class="filter-item"><i class="fas fa-baby"></i> Weaning</span>
          <span class="filter-item"><i class="fas fa-pills"></i> Supplements</span>
          <span class="filter-item"><i class="fas fa-tint"></i> Hydration</span>
        </div>
      </div>
    </div>
    <?php endif; ?>
  ...
  <div class="products-area">
    <?php if ($tab === 'products'): ?>
      <div class="results-header">
        <div class="results-count">Showing <strong><?= $total_products ?></strong> nutrition products</div>
        <div class="sort-bar">
      ...
      <?php if ($total_products === 0): ?>
        <div class="no-items"><div class="no-items-icon">🍎</div><h3 style="font-family:'Playfair Display',serif;">No products yet</h3><p style="color:var(--text-light);">Nutrition products will appear here once sellers add them.</p></div>
      <?php else: ?>
          ...
          <div class="product-card">
            ...
            <a href="product.php?id=<?= $p['id'] ?>" style="text-decoration:none;color:inherit;display:block;">
              ...
              <div class="product-body">
                <div class="product-cat"><?= htmlspecialchars($p['category_name'] ?? 'Nutrition') ?></div>
    ...
    <?php else: ?>
      <div class="results-header">
        <div class="results-count">Showing <strong><?= $total_articles ?></strong> nutrition guides</div>
      </div>
      ...
      <?php else: ?>
        <div class="articles-grid">
          ...
          <a href="article.php?id=<?= $a['id'] ?>" class="article-card">
            ...
            <div>
              <div class="article-topic"><?= htmlspecialchars($a['topic'] ?: 'Nutrition') ?></div>
